...
data_generator = Data(path=args.data_path + args.dataset, batch_size=args.batch_size, dataset = args.dataset, neg_num = args.negative_samples)
...
def test_ret_ratings(sess, model, users_to_test, drop_flag=False, batch_test_flag=False):
    ...
    for u_batch_id in range(n_user_batchs):
        ...
        if batch_test_flag:
            ...
            if ret_rate_batch == []:
                ret_rate_batch = rate_batch
            else:
                ret_rate_batch = np.append(ret_rate_batch, rate_batch, axis = 0)
